...
export default function AutomationsPage() {
  const t = useTranslations("pages.automations")
  ...
  async function toggleActive(a: Automation, next: boolean) {
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error ?? t("errors.update"))
      return
    }
    toast.success(next ? t("success.activated") : t("success.paused"))
  }
  ...
  async function duplicate(a: Automation) {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("errors.duplicate"))
      return
    }
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("errors.delete"))
      return
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {showTemplates && (
        <section>
          <h2 className="mb-3 text-sm font-semibold text-muted-foreground">{t("templates")}</h2>
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>{t("last", { time: formatRelative(automation.last_executed_at) })}</span>
          </div>
        </button>
