...
function relativeTime(
  iso: string,
  t: ReturnType<typeof useTranslations<'dashboard.activity'>>,
): string {
  const then = new Date(iso).getTime()
  if (Number.isNaN(then)) return ''
  const diffSec = Math.round((Date.now() - then) / 1000)
  if (diffSec < 60) return t('timeAgo.seconds', { count: Math.max(1, diffSec) })
  if (diffSec < 3600) return t('timeAgo.minutes', { count: Math.floor(diffSec / 60) })
  if (diffSec < 86400) return t('timeAgo.hours', { count: Math.floor(diffSec / 3600) })
  if (diffSec < 2_592_000) return t('timeAgo.days', { count: Math.floor(diffSec / 86400) })
