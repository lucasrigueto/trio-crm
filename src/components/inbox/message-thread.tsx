...
const STATUS_OPTIONS: { labelKey: "open" | "pending" | "closed"; value: ConversationStatus; color: string }[] = [
...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    const remaining =
      hoursLeft >= 1
        ? t("session.hoursRemaining", { count: Math.floor(hoursLeft) })
        : t("session.minutesRemaining", { count: Math.floor(hoursLeft * 60) });
    ...
    async (text: string, replyToId?: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(t("errors.failedToSend", { reason }));
    ...
    async (
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(t("errors.failedToSendTemplate", { reason }));
          onUpdateMessage(tempId, { status: "failed" });
          return;
        }
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error(t("errors.waitForSend"));
        return;
      }
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : "network error";
        toast.error(t("errors.reactionFailed", { reason }));
  ...
  const handleAssignChange = useCallback(
    async (agentId: string | null) => {
      ...
      if (error) {
        ...
        toast.error(t("errors.failedToUpdateAssignment"));
        return;
      }
  ...
  const assignLabel = assignedAgentId
    ? (currentAssignee?.full_name ?? t("assignment.assigned"))
    : t("assignment.assign");
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={cn(
                ...
                {currentStatus ? t(`status.${currentStatus.labelKey}`) : t("status.fallback")}
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1">
                        {p.full_name}
                        {p.user_id === user?.id ? ` (${t("assignment.me")})` : ""}
                      </span>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <p className="text-sm text-muted-foreground">{t("noMessagesTitle")}</p>
